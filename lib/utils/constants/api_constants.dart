class BaseUrlConstants{
 static const  baseURLDev = 'https://api-ccvc-uat.chinhquyendientu.vn';
  static const baseURLStaging = 'https://api-ccvc-uat.chinhquyendientu.vn';
  static const baseURLProduct = 'https://api-ccvc-uat.chinhquyendientu.vn';
  static const baseURLGetWay = 'https://api-gateway-ccvc-uat.chinhquyendientu.vn';
}
class ApiConstants {
  static const DASH_BOARD_VBDEN =
      '/qlvb/api/danh-muc/dashboard-van-ban-den';
  static const DASH_BOARD_VBDi =
      '/qlvb/api/van-ban-di/so-luong';
  static const LOGIN = '/api/auth/login';
  static const int DEFAULT_PAGE_SIZE = 45;
  static const int PAGE_BEGIN = 1;
  static const int NOT_SAVED_ID = -1;
  static const int TIME_OUT = 30;
  static const String GET_LIST_TOKEN = '/market/coin-list';
  static const String GET_PRICE_TOKEN_BY_SYMBOL = '/market/coin-price/';

}
